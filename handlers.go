...
func (s *server) handlerShortenLink(w http.ResponseWriter, r *http.Request) {
	...
	// s.logger.Info("Parsed URL: scheme",
	// 	"scheme", u.Scheme,
	// 	"host", u.Host,
	// )
	// fmt.Sprintf("Parsed URL: scheme=%s, host=%s", u.Scheme, u.Host)
	...
	// fmt.Sprintf("Generated short code: %s for URL: %s", shortCode, longURL)
...
func (s *server) handlerRedirect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			...
			s.logger.Info("failed to lookup URL",
				"error", err,
			)
			// fmt.Sprintf("failed to lookup URL: %v", err)
